...
#sort code used from book

sort:
	addi $sp,$sp,–20 #makeroomonstackfor5registers 
...
for1tst:
	slt $t0,$s0,$s3 #reg$t0=0if$s0Š$s3(iŠn)
	
	beq $t0, $zero, exit1 # go to exit1 if $s0 Š $s3 (i Š n)
...
for2tst:
	...
	slt $t0,$t4,$t3 #reg$t0=0if$t4Š$t3 
	
	beq $t0, $zero, exit2# go to exit2 if $t4 Š $t3
